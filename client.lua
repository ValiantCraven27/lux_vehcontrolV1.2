...
RegisterCommand('lightsbeep', function(source, args)
...
while true do
	Wait(5000)
	if policelights and beepenabled then
	print('lightsok')
	SendNUIMessage({
	transactionType     = 'beeplights'
	})
	end
end
end)
...
Citizen.CreateThread(function()
    while true do
local playerPed                   = GetPlayerPed(-1)
local driverPed                   = GetPedInVehicleSeat(CarID, -1)
local playerCoords             = GetEntityCoords(playerPed)
local CarID   	=  GetVehiclePedIsIn(playerPed, false)
CarClass =  GetVehicleClass(CarID)

        Citizen.Wait(0)
                            if IsVehicleSirenOn(CarID) then
SetVehicleIndicatorLights(CarID, 0, false)
SetVehicleIndicatorLights(CarID, 1, false)
                               else
